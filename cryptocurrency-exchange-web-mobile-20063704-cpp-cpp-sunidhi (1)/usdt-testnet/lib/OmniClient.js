...
Omni.listtransactions = function(address, cb){
  
     client.cmd("omni_listtransactions",address ,function(err, data, resHeaders){
   //   console.log("err,,,,,,,",err,"data,,,,,,,", data,"header........." ,resHeaders)
        if (err) return console.log(err)
...
Omni.omnifundedsend = function(address, address2, id, amount,feeAddress, cb){
  client.cmd('omni_funded_send', address, address2, id, amount,feeAddress, function(err, data, resHeaders){
      if (data){
      return cb(data)
    }else if(err){
      return cb(null,err)
